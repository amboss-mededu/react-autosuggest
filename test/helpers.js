...
export const expectHoveredSuggestion = (suggestion) => {
  const hoveredSuggestions = TestUtils.scryRenderedDOMComponentsWithClass(
    app,
    'react-autosuggest__suggestion--hovered'
  );

  if (suggestion === null) {
    expect(hoveredSuggestions).to.have.length(0);
  } else {
    expect(hoveredSuggestions).to.have.length(1);
    expect(hoveredSuggestions[0].textContent).to.equal(suggestion);
  }
};
